...
<%@ include file="/WEB-INF/views/layout/header.jsp" %>
  ...
  <h2 class="h5 section-title">Recent Post<c:if test="${!empty mlogin}">
		...
		<div class="btnBox">
			<form action="${pageContext.request.contextPath }/post/list" method="get" onsubmit="return check(this)" >
				<select name="type">
					<option value="post_title" ${param.type == 'post_title' or empty param.type ? 'selected' : '' }>제목으로 검색</option>
					<option value="id" ${param.type == 'id' ? 'selected' : '' }>아이디로 검색</option>
					<option value="post_content" ${param.type == 'post_content' ? 'selected' : '' }>내용으로 검색</option>
				</select>
